...
do
    ...
    local status, err = pcall(function()
        test:test("fill and test cat output", function(test_i)
            ...
            check_ctlcat_xlog(test_i, dir, "--from=3 --to=7 --format=json", "\n", 2)
...
if test_run == nil then
    ...
    local status, err = pcall(function()
        test:test("fill and test play output", function(test_i)
            ...
            test_i:is(lsn_before, 8, "check lsn before")
            local res, stdout, stderr = run_command(dir, command_base)
            test_i:is(res, 0, "execution result")
            test_i:is(test_run:get_lsn("remote", 1), 20, "check lsn after")
            local res, stdout, stderr = run_command(dir, command_base)
            test_i:is(res, 0, "execution result")
            test_i:is(test_run:get_lsn("remote", 1), 32, "check lsn after")
        end)
    end)
